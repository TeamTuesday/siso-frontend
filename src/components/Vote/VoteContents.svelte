...
<div class="flex-1 overflow-hidden">
	<div class="flex flex-col items-center flex-1 pt-4 pb-2 h-full" class:!h-max={changed}>
		...
		<button class="mb-3 vote-btn" class:voted={votedType} on:click={() => vote(voteType.AGREE)}>
			{#if votedType}
		...
		<button class="vote-btn" class:voted={votedType} on:click={() => vote(voteType.DISAGREE)}>
			{#if votedType}
				<span class="vote-percent" class:change={changed}>
					<Countup
						initial={0}
