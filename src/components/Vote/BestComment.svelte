<script lang="ts">
  ...
  export let data:VoteModule.BestComment = {
    id: '',
    createdAt: '',
    updateAt: '',
    deleteAt: '',
...
<div class="best-comment mt-4" class:mb-4={isAgree}>
  <div class="best-comment-header">
    <div class="flex gap-2 items-center">
      <p class="text-[18px] leading-[21px] font-bold text-[#FFD952] text-[#BD8DDB]" class:!text-[#FFD952]={!isAgree}>
        {isAgree ? 'A' : 'B'}
      </p>
      <span
      ...
      <p class="text-[12px] leading-[16px] text-[#222222] font-medium">{data && data.userId}</p>
    </div>
    <div class="flex items-center gap-1">
      <p class="text-[12px] leading-[16px] text-[#999999]">{data && data.likeCount}</p>
...
<!-- 좋아요 눌렀다면 - 기능추가도 필요 - 
  ...
  <div class="best-comment-content">{data && data.comment}
  </div>
</div>
